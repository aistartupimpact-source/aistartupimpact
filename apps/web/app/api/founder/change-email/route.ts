...
import { requireFounderAuth, verifyPassword } from '@/lib/founder-auth';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await prisma.founderUser.findUnique({ where: { email: emailLower } });
    if (existing) {
    ...
    const verifyUrl = `${process.env.NEXT_PUBLIC_WEB_URL || 'https://aistartupimpact.com'}/api/verify-email-change?token=${token}&type=founder`;
