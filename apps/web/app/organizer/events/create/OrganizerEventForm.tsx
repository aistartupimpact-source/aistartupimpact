      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:items-start">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <Section title="Basic Information" icon={ImageIcon} open={openSections.basic} onToggle={()=>toggle("basic")}>
            {/* Cover - compact */}
            <div>
              <label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Event Cover * <span className="normal-case">Square · JPG/PNG/WebP · 5MB</span></label>
              {coverImageUrl ? (
                <div className="flex items-center gap-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                  ...
                  <span className="text-xs text-green-600 font-jakarta font-medium flex-1">✓ Uploaded</span>
                  <button type="button" onClick={()=>fileRef.current?.click()} className="text-xs text-brand font-jakarta hover:underline">Change</button>
                  <button type="button" onClick={()=>setCoverImageUrl("")} className="text-xs text-red-500 font-jakarta hover:underline">Remove</button>
                </div>
              ) : (
              ...
              {fieldErrors.cover && <p className="text-xs text-red-500 font-jakarta mt-1">{fieldErrors.cover}</p>}
            </div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Title *</label><input required value={title} onChange={e=>{setTitle(e.target.value);if(!customSlug)setSlug(slugify(e.target.value+(city?`-${city}`:"")));if(fieldErrors.title)setFieldErrors(p=>({...p,title:""}))}} placeholder="AI Builders Hackathon" className={`${ic} ${fieldErrors.title?"!border-red-400 !ring-red-200":""}`} maxLength={200}/>{fieldErrors.title&&<p className="text-xs text-red-500 font-jakarta mt-0.5">{fieldErrors.title}</p>}<p className="text-xs text-gray-400 mt-0.5 font-jakarta">Appears everywhere attendees see your event</p></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Short Description</label><input value={subtitle} onChange={e=>setSubtitle(e.target.value)} placeholder="Join 500+ AI founders" className={ic} maxLength={300}/></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Category *</label><div className="grid grid-cols-4 gap-1">{CATEGORIES.map((c,i)=><button key={c} type="button" onClick={()=>setCategory(CATEGORY_VALUES[i])} className={`px-2 py-1.5 rounded text-xs font-jakarta font-medium border ${category===CATEGORY_VALUES[i]?"border-brand bg-brand/5 text-brand":"border-gray-200 dark:border-gray-700 text-gray-500"}`}>{c}</button>)}</div></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Tags <span className="normal-case">(comma-separated)</span></label><input value={tags} onChange={e=>setTags(e.target.value)} placeholder="AI, GenAI, Startups, LLMs" className={ic}/></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Slug</label>
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1.5 cursor-pointer shrink-0"><input type="checkbox" checked={customSlug} onChange={e=>setCustomSlug(e.target.checked)} className="w-3.5 h-3.5 rounded text-brand focus:ring-brand"/><span className="text-xs text-gray-500 font-jakarta">Custom</span></label>
              ...
              <p className="text-xs text-gray-400 font-jakarta mt-0.5">/events/{customSlug&&slug?slug:generatedSlug||"..."}</p>
            </div>
          </Section>
          ...
          <Section title="Schedule" icon={Clock} open={openSections.schedule} onToggle={()=>toggle("schedule")}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs text-gray-400 mb-0.5 font-jakarta">Start *</label><input type="datetime-local" required value={startAt} onChange={e=>{setStartAt(e.target.value);if(sameDay&&e.target.value)setEndAt(e.target.value.split("T")[0]+"T"+(endAt.split("T")[1]||"18:00"));if(fieldErrors.startAt)setFieldErrors(p=>({...p,startAt:""}))}} className={`${ic} ${fieldErrors.startAt?"!border-red-400":""}`}/>{fieldErrors.startAt&&<p className="text-xs text-red-500 font-jakarta mt-0.5">{fieldErrors.startAt}</p>}</div>
              <div><label className="block text-xs text-gray-400 mb-0.5 font-jakarta">End *</label><input type="datetime-local" required value={endAt} onChange={e=>{setEndAt(e.target.value);if(fieldErrors.endAt)setFieldErrors(p=>({...p,endAt:""}))}} className={`${ic} ${fieldErrors.endAt?"!border-red-400":""}`}/>{fieldErrors.endAt&&<p className="text-xs text-red-500 font-jakarta mt-0.5">{fieldErrors.endAt}</p>}</div>
            </div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Timezone *</label><select value={timezone} onChange={e=>setTimezone(e.target.value)} className={ic}>{TIMEZONES.map(t=><option key={t.v} value={t.v}>{t.l}</option>)}</select></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Registration Deadline <span className="normal-case text-gray-400">(optional)</span></label><input type="datetime-local" value={registrationDeadline} onChange={e=>setRegistrationDeadline(e.target.value)} className={ic}/></div>
          </Section>

          {/* ═══ 3. LOCATION ═══ */}
          <Section title="Location" icon={MapPin} open={openSections.location} onToggle={()=>toggle("location")}>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Format *</label><div className="grid grid-cols-3 gap-2">{[{v:"IN_PERSON",l:"In Person",I:MapPin},{v:"VIRTUAL",l:"Virtual",I:Video},{v:"HYBRID",l:"Hybrid",I:Globe}].map(f=><button key={f.v} type="button" onClick={()=>setFormat(f.v)} className={`px-3 py-2 rounded-lg text-center border ${format===f.v?"border-brand bg-brand/5":"border-gray-200 dark:border-gray-700"}`}><f.I className={`w-3.5 h-3.5 mx-auto mb-0.5 ${format===f.v?"text-brand":"text-gray-400"}`}/><p className={`text-xs font-jakarta ${format===f.v?"text-brand font-medium":"text-gray-500"}`}>{f.l}</p></button>)}</div></div>
            {showPhysical && <>
              ...
              <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">City</label><CitySelect value={city} onChange={(c) => { setCity(c.name); if(!customSlug) setSlug(slugify(title+(c.name?`-${c.name}`:""))) }} placeholder="Search city..." /></div>
          ...
          <Section title="Registration" icon={Users} open={openSections.registration} onToggle={()=>toggle("registration")}>
            <div className="space-y-3">
              <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Capacity</label>
              ...
              <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Visibility</label>
              ...
              <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Custom Registration Fields</label><button type="button" onClick={()=>setCustomQuestions(p=>[...p,{text:"",type:"TEXT",required:false,options:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add Field</button></div>
                <p className="text-xs text-gray-400 font-jakarta mb-2">Default fields: Name, Email, Phone, Occupation, City. Add custom fields below.</p>
                  ...
                  <div key={i} className="flex gap-2 mb-2 items-start p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div className="flex-1 space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer shrink-0"><input type="checkbox" checked={q.required} onChange={e=>{const u=[...customQuestions];u[i]={...u[i],required:e.target.checked};setCustomQuestions(u)}} className="w-3 h-3 rounded text-brand"/><span className="text-xs text-gray-500 font-jakarta">Required</span></label>
                      </div>
              ...
              {/* Ticket Types */}
              <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Ticket Types</label><button type="button" onClick={()=>setTicketTiers(p=>[...p,{name:"General",type:"REGULAR",quantity:"",description:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add Tier</button></div>
                <p className="text-xs text-gray-400 font-jakarta mb-2">All events are free. Use tiers for capacity limits (VIP seating, student quota, etc.)</p>
                {ticketTiers.map((t,i)=>(
                  <div key={i} className="flex gap-2 mb-2 items-start p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              {/* Check-in Settings */}
              <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Check-in Method</label>
          ...
          <Section title="Content" icon={Mic} open={openSections.content} onToggle={()=>toggle("content")}>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Description</label><textarea value={description} onChange={e=>{if(e.target.value.length<=MAX_DESC)setDescription(e.target.value)}} rows={5} placeholder="What's this event about? Use markdown: **bold**, *italic*, - bullets, [links](url)" className={ic+" resize-y"} maxLength={MAX_DESC}/><div className="flex justify-between mt-0.5"><p className="text-xs text-gray-400 font-jakarta">Markdown supported: **bold** *italic* - lists [links](url)</p><p className={`text-xs font-jakarta ${description.length>MAX_DESC*0.9?"text-red-500":"text-gray-400"}`}>{description.length}/{MAX_DESC}</p></div></div>
            {/* Speakers */}
            <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Speakers</label><button type="button" onClick={()=>setSpeakers(p=>[...p,{name:"",title:"",company:"",linkedin:"",photo:"",photoError:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add Speaker</button></div>
              {speakers.length===0?<p className="text-xs text-gray-400 font-jakarta">No speakers added yet</p>:speakers.map((s,i)=>(
                <div key={i} className="mb-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                  <div className="flex items-start justify-between mb-2">
                    <span className="text-xs font-bold text-gray-400 font-jakarta">Speaker {i+1}</span>
                  ...
                  {/* Photo Upload */}
                  <div className="mb-3">
                    <label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Photo <span className="normal-case text-gray-400">JPG/PNG/WebP · Max 2MB · Square recommended</span></label>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <label htmlFor={`speaker-photo-${i}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-600 rounded-lg text-xs font-semibold text-gray-600 dark:text-gray-300 font-jakarta cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                          <Upload className="w-3 h-3"/> {s.photo ? "Change Photo" : "Upload Photo"}
                        </label>
                        {s.photo && <button type="button" onClick={()=>{const u=[...speakers];u[i]={...u[i],photo:"",photoError:""};setSpeakers(u)}} className="ml-2 text-xs text-red-500 font-jakarta hover:underline">Remove</button>}
                        {s.photoError && <p className="text-xs text-red-500 font-jakarta mt-1">{s.photoError}</p>}
                      </div>
                    </div>
                  </div>
                  {/* Name */}
                  <div className="mb-2"><label className="block text-xs font-bold text-gray-400 uppercase mb-0.5 font-jakarta">Full Name *</label><input value={s.name} onChange={e=>{const u=[...speakers];u[i]={...u[i],name:e.target.value};setSpeakers(u)}} placeholder="John Doe" className={ic}/></div>
                  {/* Role + Company */}
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div><label className="block text-xs font-bold text-gray-400 uppercase mb-0.5 font-jakarta">Role</label><input value={s.title} onChange={e=>{const u=[...speakers];u[i]={...u[i],title:e.target.value};setSpeakers(u)}} placeholder="CTO, Founder" className={ic}/></div>
                    <div><label className="block text-xs font-bold text-gray-400 uppercase mb-0.5 font-jakarta">Company</label><input value={s.company} onChange={e=>{const u=[...speakers];u[i]={...u[i],company:e.target.value};setSpeakers(u)}} placeholder="Company name" className={ic}/></div>
                  </div>
                  {/* LinkedIn */}
                  <div><label className="block text-xs font-bold text-gray-400 uppercase mb-0.5 font-jakarta">LinkedIn</label><input value={s.linkedin} onChange={e=>{const u=[...speakers];u[i]={...u[i],linkedin:e.target.value};setSpeakers(u)}} placeholder="https://linkedin.com/in/..." className={ic}/></div>
                </div>
              ))}
            </div>
            {/* Agenda */}
            <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Agenda</label><button type="button" onClick={()=>setAgenda(p=>[...p,{time:"",title:"",speaker:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add</button></div>
              {agenda.length===0?<p className="text-xs text-gray-400 font-jakarta">No agenda items</p>:agenda.map((a,i)=>(
            ...
            {/* Sponsors */}
            <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Sponsors</label><button type="button" onClick={()=>setSponsors(p=>[...p,{name:"",tier:"GOLD",website:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add</button></div>
              {sponsors.length===0?<p className="text-xs text-gray-400 font-jakarta">No sponsors added</p>:sponsors.map((s,i)=>(
            ...
            {/* Community Partners */}
            <div><div className="flex items-center justify-between mb-1"><label className="text-xs font-bold text-gray-400 uppercase font-jakarta">Community Partners</label><button type="button" onClick={()=>setPartners(p=>[...p,{name:"",website:""}])} className="text-xs text-brand font-semibold font-jakarta flex items-center gap-0.5"><Plus className="w-3 h-3"/>Add</button></div>
              {partners.length===0?<p className="text-xs text-gray-400 font-jakarta">No partners added</p>:partners.map((p2,i)=>(
          ...
          <Section title="SEO & Sharing" icon={Eye} open={openSections.seo} onToggle={()=>toggle("seo")}>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">SEO Title <span className="normal-case">(defaults to event title)</span></label><input value={seoTitle} onChange={e=>setSeoTitle(e.target.value)} placeholder={title||"Event title"} className={ic} maxLength={70}/><p className="text-xs text-gray-400 text-right font-jakarta">{(seoTitle||title).length}/70</p></div>
            <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">SEO Description</label><textarea value={seoDescription} onChange={e=>setSeoDescription(e.target.value)} placeholder={subtitle||"Auto-generated from short description"} className={ic+" resize-none"} rows={2} maxLength={160}/><p className="text-xs text-gray-400 text-right font-jakarta">{(seoDescription||subtitle).length}/160</p></div>
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
              <p className="text-xs font-bold text-gray-400 uppercase mb-1 font-jakarta">Preview URL</p>
              <p className="text-xs text-brand font-jakarta font-mono">/events/{customSlug&&slug?slug:generatedSlug||"..."}</p>
              <p className="text-xs text-gray-400 font-jakarta mt-2">Short URL auto-generated after publish (e.g. /e/KX7M4P)</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/10 rounded-lg p-3 border border-blue-100 dark:border-blue-900/20">
              <p className="text-xs font-bold text-blue-600 uppercase mb-0.5 font-jakarta">📅 Calendar Integration</p>
              <p className="text-xs text-blue-600/80 font-jakarta">Google Calendar, Outlook, and .ics downloads are auto-generated for attendees after registration.</p>
            </div>
          </Section>
        ...
        <div className="hidden lg:block lg:col-span-1">
          <div className="sticky top-6">
            <p className="text-xs font-bold uppercase text-gray-400 font-jakarta mb-2 tracking-wider">Live Preview</p>
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
              <div className="relative" style={{aspectRatio:"1/1"}}>{coverImageUrl?<Image src={coverImageUrl} alt="" width={400} height={400} unoptimized className="w-full h-full object-cover"/>:<div className="w-full h-full bg-gradient-to-br from-brand/5 to-brand/10 flex items-center justify-center"><ImageIcon className="w-7 h-7 text-gray-200"/></div>}<span className="absolute top-2 left-2 text-xs font-bold uppercase bg-white/90 text-brand px-1.5 py-0.5 rounded-full">{CATEGORIES[CATEGORY_VALUES.indexOf(category)]}</span></div>
              <div className="p-3 space-y-1.5">
                ...
                {subtitle&&<p className="text-xs text-gray-500 font-jakarta">{subtitle}</p>}
                {startAt&&<p className="text-xs text-gray-500 font-jakarta flex items-center gap-1"><Calendar className="w-3 h-3 text-brand"/>{new Date(startAt).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}</p>}
                <p className="text-xs text-gray-500 font-jakarta flex items-center gap-1">{format==="VIRTUAL"?<><Video className="w-3 h-3 text-purple-500"/>Online</>:city||venueName?<><MapPin className="w-3 h-3 text-blue-500"/>{city||venueName}</>:<><MapPin className="w-3 h-3 text-gray-300"/>TBA</>}</p>
                {tags&&<div className="flex flex-wrap gap-1">{tags.split(",").slice(0,4).map((t,i)=>t.trim()&&<span key={i} className="text-xs px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-500 rounded font-jakarta">{t.trim()}</span>)}</div>}
                <div className="flex items-center justify-between pt-1.5 border-t border-gray-100 dark:border-gray-800"><span className="text-xs font-bold text-green-600">Free</span><span className="text-xs text-gray-400 flex items-center gap-0.5"><Users className="w-2.5 h-2.5"/>{unlimited?"∞":capacity||"∞"}</span></div>
                {speakers.length>0&&<div className="pt-1"><p className="text-xs text-gray-400 font-jakarta">{speakers.length} speaker{speakers.length>1?"s":""}</p></div>}
                <div className="pt-1"><div className="w-full py-1.5 bg-brand/10 text-brand text-xs font-bold font-jakarta rounded text-center">Register Now</div></div>
              </div>
            </div>
          </div>
        </div>
