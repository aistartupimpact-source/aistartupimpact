...
export default function CityCombobox({
  ...
  useEffect(() => {
    async function fetchDbCities() {
      ...
      try {
        ...
        console.error('Failed to load database cities in admin:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Merge static database and dynamic database cities
    ...
    <div ref={containerRef} className="relative w-full text-left">
      <div className="relative">
        <input
          ...
          autoComplete="off"
          className={`w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent ${className}`}
        />
            ...
            return (
              <li
                ...
                className={`relative cursor-pointer select-none py-2 pl-10 pr-4 transition-colors ${
                  isActive
                    ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-white'
                    : isSelected
